  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 overflow-y-auto px-2 ${isCollapsed ? 'flex flex-col items-center' : ''} scrollbar-hide hover:scrollbar-default`}>
        ...
        {/* List */}
        <div className={`pb-4 ${isCollapsed ? 'flex flex-col items-center gap-4 w-full' : (isExpanded ? "grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-4 px-4 mt-4" : "flex flex-col gap-1")}`}>
          {!isAuthenticated ? (
            !isCollapsed && (
