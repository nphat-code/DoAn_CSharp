  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 overflow-y-auto px-2 ${isCollapsed ? 'flex flex-col items-center' : ''} scrollbar-hide hover:scrollbar-default`}>
        ...
        {/* List */}
        <div className={`pb-4 ${isCollapsed ? 'flex flex-col items-center gap-4 w-full' : (isExpanded ? "grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-0 px-4 mt-4" : "flex flex-col gap-1")}`}>
          {!isAuthenticated ? (
            !isCollapsed && (
          ...
          {isAuthenticated && playlists.map(playlist => (
            <div 
              ...
              className={`group/item relative hover:bg-[#282828] rounded-md cursor-pointer transition ${isCollapsed ? 'p-0 w-12 h-12 flex justify-center items-center shrink-0' : (isExpanded ? 'flex flex-col items-start gap-3 bg-transparent p-4' : 'p-2 flex items-center gap-3')}`}
              title={playlist.name}
            >
