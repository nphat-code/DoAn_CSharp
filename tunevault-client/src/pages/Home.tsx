      ...
      {currentUser && (activeTab === 'all' || activeTab === 'foryou') && aiTracks.length > 0 && (
        <section className="mb-10">
          <div className="flex items-end justify-between mb-4 mt-2">
            <div>
              <h2 className="text-2xl font-bold text-white hover:underline cursor-pointer">Dành cho {currentUser.username}</h2>
            </div>
            {activeTab === 'all' && (
              <span onClick={() => setActiveTab('foryou')} className="text-sm font-bold text-zinc-400 hover:text-white cursor-pointer transition">Hiện tất cả</span>
            )}
          </div>
          ...
          <div className={activeTab === 'all' ? "flex overflow-x-auto gap-6 pb-4 custom-scrollbar" : "grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-6"}>
            {aiTracks.slice(0, activeTab === 'all' ? 10 : undefined).map(track => (
              <div 
                key={track.id}
