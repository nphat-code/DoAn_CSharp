...
export const SharedWithMe: React.FC = () => {
  ...
  const handlePlayClick = async (e: React.MouseEvent, share: MediaShareDto) => {
    e.stopPropagation();
    const type = share.mediaType.toLowerCase();
    ...
    if (type === 'track' || type === 'audio' || type === 'video') {
      ...
      if (currentMedia?.albumId === share.mediaItemId) {
        togglePlayPause();
        return;
      }
      try {
        const albumDetail = await albumService.getAlbumById(share.mediaItemId);
        if (albumDetail.tracks && albumDetail.tracks.length > 0) {
          const tracksToPlay = albumDetail.tracks.map((t: any) => ({
            ...t,
            albumId: share.mediaItemId,
            isAlbumContext: true,
      ...
      if (currentMedia?.playlistId === share.mediaItemId) {
        togglePlayPause();
        return;
      }
      try {
        const playlistDetail = await playlistService.getPlaylistDetails(share.mediaItemId);
        if (playlistDetail.tracks && playlistDetail.tracks.length > 0) {
          const tracksToPlay = playlistDetail.tracks.map((t: any) => ({
            ...t,
            playlistId: share.mediaItemId,
            coverUrl: t.coverUrl,
