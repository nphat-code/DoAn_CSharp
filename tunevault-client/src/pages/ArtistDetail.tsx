...
export const ArtistDetail = () => {
  ...
  return (
    <div className="flex flex-col h-full relative bg-spotify-card overflow-hidden">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col justify-end min-w-0 flex-1 w-full pb-1">
            ...
            <div className="text-white font-medium text-base drop-shadow-md">
              {(Math.floor((artist.name.length * 1234567) % 99900000 + 100000) + (artist.realMonthlyListeners || 0)).toLocaleString('vi-VN')} người nghe hằng tháng
            </div>
          </div>
        </div>
      ...
      {showShareModal && (
        <ShareMediaModal
          mediaId={shareData ? shareData.id : artist.id}
          mediaType={shareData ? shareData.type : "Nghệ sĩ"}
          mediaTitle={shareData ? shareData.title : artist.name}
          onClose={() => {
